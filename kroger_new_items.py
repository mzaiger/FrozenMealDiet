...
find 20 new ones. For every Kroger product pulled:
  ...
  5. Best score 90 or below -> it's new, and gets a new pool object:
       ...
       - Walmart page: Groq's free openai/gpt-oss-20b model, with its
         built-in browser_search tool (console.groq.com/docs/browser-search,
         powered by Exa), is asked to find the walmart.com product page for
         "<brand> <name>". Each candidate URL it returns is checked for the
         real /ip/<slug>/<id> product-page shape AND for being the SAME
         PRODUCT as the Kroger one: the product name in the URL must carry
         Kroger's brand and score at least groq_search.min_name_match_score
         (75) against Kroger's name. The model can still surface the
         closest Walmart page rather than the exact one, which is often a
         different flavor or brand; the first candidate that passes wins,
         and if none does the product is skipped ("no_matching_walmart_page").
         The URL (+ ?fulfillmentIntent=Pickup) goes in PRODUCT_URL and the
         numeric id in SKU. If that SKU is already on an ACTIVE pool item the
  ...
  6. "active" is True on new records: Groq's browser search already
     ...
     details are kept in "_groq_search_*" fields, separate from that
...
run through the (quota-limited) Groq/DDG steps in one run, and
...
Env vars required:
    ...
    GROQ_API_KEY                            -- console.groq.com API key,
                                                free tier (openai/gpt-oss-20b
                                                + its browser_search tool)
...
def tag_existing_pool_item_with_upc(pool, sku, upc):
    ...
    later pull, instead of being re-resolved through Groq (a real,
...
def revive_pool_item(item, record, upc, run_date):
    ...
    when the Walmart page Groq just found has the same SKU as a row
    that was marked inactive (or never checked): the product IS that row,
    and Groq has just proved the page is live. Every field of `record`
...
def load_skipped_upcs(path):
    ...
    they aren't re-tried (and don't re-spend Groq quota) every run.
...
# ---------------------------------------------------------------------------
# Step 2: Groq (openai/gpt-oss-20b + browser_search) Walmart link lookup
# ---------------------------------------------------------------------------
...
# Any http(s) URL embedded in the model's free-form answer -- a browser-
# search response is prose, not a structured result list, so URLs may sit
...
def extract_walmart_candidate_urls(text, max_urls):
    ...
    search response's free-form text, in the order they first appear.
    Keeps only walmart.com/.walmart.com links with the real
    /ip/<slug>/<id> product-page shape (same regex Serper-based results
    were checked against), de-duplicated by canonical URL. Returns a list
    of (canonical_url, slug, sku) tuples, most-likely-match first (i.e. in
    the order the model mentioned them), up to max_urls."""
    seen = set()
    out = []
    for m in _ANY_URL_RE.finditer(text or ""):
        url = m.group(0).rstrip(".,;:!?\u3002")
        netloc = urlparse(url).netloc.lower().split(":")[0]
        if netloc != "walmart.com" and not netloc.endswith(".walmart.com"):
            continue
        ip_match = WALMART_IP_URL_RE.search(urlparse(url).path)
        if not ip_match:
            continue
        slug, sku = ip_match.group(1), ip_match.group(2)
        canon = f"https://www.walmart.com/ip/{slug}/{sku}"
        if canon in seen:
            continue
        seen.add(canon)
        out.append((canon, slug, sku))
        if len(out) >= max_urls:
            break
    return out
...
def walmart_page_matches(candidate, slug_name, cfg):
    ...
    as this Kroger candidate? Groq's browser search only returns the
    closest Walmart page(s) it found, which can be a different flavor or
    even a different brand -- and treating a wrong page as this product
    would tag its UPC on the wrong pool row or overwrite the wrong row.
    Returns (matches, score, reason). It matches when:
      ...
      2. the cleaned names score at least groq_search.min_name_match_score
    ...
    if score < cfg["groq_search"]["min_name_match_score"]:
...
def find_walmart_listing(candidate, api_key, cfg):
    ...
    name>". Replaces the old Serper.dev "site:walmart.com ..." Google
    search: groq/compound and groq/compound-mini (the earlier built-in-
    web-search models) were decommissioned on 2026-09-21, and
    browser_search on a plain chat model is Groq's current free-tier
    replacement for that kind of lookup. The model is asked to list up to
    groq_search.max_candidate_urls plausible Walmart URLs, most-likely
    match first, one per line -- its answer is prose, not a structured
    result list, so extract_walmart_candidate_urls() pulls the URLs back
    out and only keeps ones on walmart.com with the real /ip/<slug>/<id>
    product-page shape. Each candidate is checked in order for being the
    SAME PRODUCT as the Kroger candidate (walmart_page_matches(): same
    brand, similar name -- judged from the product name inside the URL),
    rather than trusting whichever one the model listed first. Returns a
    ...
    prompt = (
        ...
        "this exact grocery product.\n\n"
        f"Brand: {candidate['brand'] or '(none given)'}\n"
        f"Product: {query}\n\n"
        f"Reply with ONLY a plain list of up to {max_urls} walmart.com product "
        "page URLs (https://www.walmart.com/ip/...) that could plausibly be "
        "this exact product, most likely match first, one per line, and "
        "nothing else -- no commentary, no markdown, no numbering. If you "
        "can't find a walmart.com page for this product, reply with the "
        "single word NONE."
    )
    payload = {
        "model": model,
        ...
        "reasoning_effort": gcfg.get("reasoning_effort", "low"),
        "temperature": 0.2,
        "stream": False,
    }
    ...
    candidates = extract_walmart_candidate_urls(content, max_urls)
    if not candidates:
        # Distinguishes "model found nothing at all" from "model mentioned
        # walmart.com but never a real /ip/<slug>/<id> product-page URL".
        saw_walmart_domain = "walmart.com" in content.lower()
        reason = "walmart_domain_but_no_ip_pattern" if saw_walmart_domain else "no_search_results"
        return {
            "product_url": None, "sku": None, "product_name": None, "query": query,
            "reason": reason, "top_result_url": None,
        }

    top_result_url = candidates[0][0]
    closest_rejected = None          # (name, score) of the best product page that wasn't the same product
    for url, slug, sku in candidates:
    ...
    return {
        "product_url": None, "sku": None, "product_name": None, "query": query,
        "reason": "no_matching_walmart_page", "top_result_url": top_result_url,
...
def build_pool_record(candidate, walmart, image_url, image_query, image_reason,
    ...
    record = {
        ...
        "active": True,  # Groq's browser search already confirmed a live walmart.com
        ...
        "_groq_search_query": walmart["query"],
        "_groq_search_reason": walmart["reason"],
        "_groq_search_top_result_url": walmart["top_result_url"],
...
class AbortRun(Exception):
    ...
    (e.g. Groq rejecting the API key) -- stop the run instead of burning
    through the list."""
...
_PERMANENT_WALMART_REASONS = {
    "no_search_results", "walmart_domain_but_no_ip_pattern",
    ...
    if not walmart["product_url"]:
        ...
        if walmart.get("closest_rejected"):
            ...
            detail += (f" -- closest Walmart page was a different product: {name!r} "
                       f"(name match {score:.0f}, needs {cfg['groq_search']['min_name_match_score']})")
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true",
                         help="Pull Kroger and fuzzy-match only: list the new products it would enrich, "
                              "write nothing (no UPC tagging, no pool changes), skip Groq/DDG/USDA/OFF/Gemini")
    ...
    if not args.dry_run:
        required += [("GROQ_API_KEY", groq_key), ("GEMINI_KEY", gemini_key), ("USDA_API_KEY", usda_key)]
